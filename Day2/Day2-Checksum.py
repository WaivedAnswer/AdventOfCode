...
def Checksum(spreadsheet):
    checksum = 0;
    for row in spreadsheet:
        checksum += LargestDifference(row);
    return checksum;
    
    
    
def parseSpreadSheetString(spreadsheetString):
    spreadsheet = [];
    rowStrings = spreadsheetString.split("+");
    for rowString in rowStrings:
        rowItemStrings = rowString.split("\t");
        row = [];
        for item in rowItemStrings:
            item.strip();
            row.append(int(item));
...
#print LargestDifference(spreadsheet[0]) == 8;
#print LargestDifference(spreadsheet[1]) == 4;
#print LargestDifference(spreadsheet[2]) == 6;
#print Checksum(spreadsheet) == 18;

#spreadsheet = [[5,9,2,8],[9,4,7,3],[3,8,6,5]];


#print EvenlyDividable(spreadsheet[0]) == 4;
#print EvenlyDividable(spreadsheet[1]) == 3;
#print EvenlyDividable(spreadsheet[2]) == 2;
#print Checksum2(spreadsheet) == 9;

spreadsheetString = "409	194	207	470	178	454	235	333	511	103	474	293	525	372	408	428 +\
...
spreadsheet = parseSpreadSheetString(spreadsheetString);
